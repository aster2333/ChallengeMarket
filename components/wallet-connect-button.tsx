        ...
        <Button variant="outline" className="min-w-[140px] justify-between">
          {isConnected && selectedWallet && selectedAccount ? (
            ...
            <>
              ...
              <span>{t('wallet.connect_wallet')}</span>
              <ChevronDown className="ml-2 h-4 w-4" />
            </>
          )}
        </Button>
      ...
      <DropdownMenuContent align="end" className="w-[280px]">
        {wallets.length === 0 ? (
          <p className="text-sm text-muted-foreground p-3 text-center">
            {t('wallet.no_wallets_detected')}
          </p>
        ) : (
          <>
            {!isConnected ? (
              <>
                <DropdownMenuLabel>{t('wallet.available_wallets')}</DropdownMenuLabel>
                <DropdownMenuSeparator />
              ...
              selectedAccount && (
                <>
                  <DropdownMenuLabel>{t('wallet.connected_wallet')}</DropdownMenuLabel>
                  <DropdownMenuSeparator />
